...
class AmazonCa extends Resource {

  constructor() {
    super()

    this._id = 2
    this._name = 'amazon.ca'
    this._url = 'https://www.amazon.ca/'
    this._pre_id_url = 'https://www.amazon.ca/dp/'
    this._alias = 'amazon_ca'
  ...
  find_product(page, database_product){
    return new Promise(async(resolve) => {
      ...
      const amazon_request = `https://www.amazon.ca/s?k=${utilites.encode_to_request(search_request)}&ref=nb_sb_noss`;
      ...
      try{
        ...
        if (typeof product_url !== 'undefined') {
          if (product_url.includes('dp/')) {
            ...
            const _url = 'https://www.amazon.ca' + product_url_arr[0] + id;
      ...
      console.log(err)
      output.success = false;
      output.error = 'error find_product try';
      resolve(output);return;
    }
    })
  }
