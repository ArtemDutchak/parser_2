...
class BBBCa extends Resource {

  constructor() {
    super()

    this._id = 6
    this._name = 'bedbathandbeyond.ca'
    this._url = 'https://www.bedbathandbeyond.ca/'
    this._pre_id_url = 'https://www.bedbathandbeyond.ca/store/product/id/'
    this._alias = 'bedbathandbeyond_ca'
  }
  ...
  get_category_products(page, category_url){
    return new Promise(async(resolve) => {
      ...
      try {
        ...
        let first_page_url = await page.url();
        first_page_url = first_page_url.replace('/1-12', '').replace('/1-24', '').replace('/1-48', '').replace('/1-72', '').replace('/1-96', '');
        first_page_url = first_page_url + '/1-96';
  ...
  parse_category_grid($){
    const out = [];
    const cell_selector = '.ProductGrid-inline_4jhKRz article';
