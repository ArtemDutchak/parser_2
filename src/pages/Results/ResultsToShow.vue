<template>
  <div class="page">
    <div id="page_back"
      :class="{show:show_collapse_filters || show_collapse_selected}"
      @click="show_collapse_filters = false;show_collapse_selected = false"
    ></div>
    <b-row class="mb-3">
                ...
                <tr>
                  ...
                  <td><b-form-input
                    id="input_max_profit_rel"
                     ...
                     size="sm"
                     @change="format_input('max_dif_percentage', 0)"
      ...
      <b-collapse class="mt-2" id="collapse_selected" v-model="show_collapse_selected">
        <b-card>
          <h5>{{ $t(`titles.selected_pairs_actions`) }}</h5>
          <hr>
          <b-row class="mb-3">
            <b-col cols="5">
              <b-button size="sm"
                class="mr-3"
                variant="light" @click="export_show_match()">
                <b-icon class="mr-1" icon="file-earmark-spreadsheet" variant="light"></b-icon>
                {{$t('title.export')}}
              </b-button>
            </b-col>
          </b-row>
          ...
          <b-row class="mb-3">
            <b-col cols="5">
              <div class="flex-row mb-2">
                <h6>{{ $t(`titles.move_or_copy_to`) }}</h6>
              </div>
              <div class="flex-row mb-2">
                <div class="f-max">
                  <b-form-select size="sm" v-model="list_id_selected" :options="list_options"></b-form-select>
                </div>
              </div>
              <div class="flex-row" v-if="list_id_selected == 0">
                <div class="f-max">
                  <b-form-input size="sm" v-model="new_list_name" :placeholder="$t('placeholder.new_list_name')"></b-form-input>
                </div>
              </div>
            </b-col>
          </b-row>
          <b-row class="mb-3">
            <b-col cols="5">
              <b-button size="sm"
                ...
                variant="light"
                 @click="move_selected">
                <b-icon class="mr-1" icon="folder-symlink" variant="light"></b-icon>
                {{$t('title.move')}}
              </b-button>
              <b-button size="sm"
                variant="light"
                 @click="copy_selected">
                <b-icon class="mr-1" icon="back" variant="light"></b-icon>
                {{$t('title.copy')}}
              </b-button>
            </b-col>
          </b-row>
          ...
          <b-row class="mb-3">
            <b-col cols="12" class="text-right">
              <b-button size="sm"
                ...
                <b-icon class="mr-1" icon="x" variant="light"></b-icon>
                {{$t('title.delete')}}
              </b-button>
            </b-col>
          </b-row>
      ...
      <b-col cols="12">
        ...
        <b-button size="sm"
          @click="collapse_selected"
          variant="light"
          :disabled="!some_selected">
          <b-icon icon="check-square" variant="light"></b-icon>
        </b-button>
      </b-col>
    </b-row>
...
export default {
  ...
  methods:{
    move_selected(){
      ...
      const showed_list_ids = this.$store.getters.showed_list_ids
      const pair_ids = []
      for (const pair of selected) {
        pair_ids.push(pair._id)
      }
      if (pair_ids.length) {
        ipcRenderer.send(`move_pairs`, {
          showed_list_ids,
          list_id: this.list_id_selected,
          list_name: this.new_list_name || this.$t('title.new_list') + " " + utilites.get_date(),
          pair_ids,
        })
      }
    },
    ...
    scan_selected(){
      ...
      for (const pair of selected) {
        pairs.push({
          product_1_id:pair.product_1._id,
          product_2_id:pair.product_2._id
        })
      }
      if (pairs.length) {
        ipcRenderer.send(`scan_pairs`, pairs)
      }
    },
    delete_selected(){
      const selected = this.get_selected()
    ...
    collapse_selected(){
      this.show_collapse_selected = !this.show_collapse_selected
      if (this.show_collapse_selected) {
        this.new_list_name = this.$t('title.new_list') + " " + utilites.get_date()
      }
    },
    set_relevance(hours){
    ...
    toggle_state(type, index){
      ...
      if (type == 'amazon_listing' && !this.filtered_results[index].match) {
        let action = [
          {
            text : this.$t('buttons.mark_match'),
            class: 'text-white',
            onClick : (e, toastObject) => {
              toastObject.goAway(0);
              setTimeout(()=>{
                ipcRenderer.send(`toggle_state`,{type:'match',state:true,_id_1,_id_2});
                ipcRenderer.send(`toggle_state`,{type:'amazon_listing',state:true,_id_1,_id_2});
              },300)
            }
          },
          {
        ...
        this.$toasted.options.action = action;
        this.$toasted.success(this.$t('toast.must_match')).goAway(5000);
      }else{
  ...
  watch:{
    currentPage(){
      ipcRenderer.send(`set_result_page`, this.$store.getters.showed_list_ids, this.currentPage)
    }
  },
  mounted(){
  ...
  destroyed(){
    this.$store.commit('set_showed_list_ids', [])
